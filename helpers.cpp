...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	for (int row = 0; row < height; row++) {
		for (int column = 0; column < width; column++) {
			newGrid[row][column] =  grid[row][column] / sum;
		}
	}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// 3 float variables to calculate blurring factor
	float centerCellProbability = 1.0 - blurring;
	float otherCellsProbability = blurring / 12.0;
	float neighbouredCellsProbability = blurring / 6.0;
	...
	vector <vector <float>> patternMatrix = {
		{otherCellsProbability, neighbouredCellsProbability, otherCellsProbability},
		{neighbouredCellsProbability, centerCellProbability, neighbouredCellsProbability},
		{otherCellsProbability,neighbouredCellsProbability,otherCellsProbability}
	};

	for (int row = 0; row < height; row++) {
		for (int column = 0; column < width; column++) {
			float value = grid[row][column];
			for (int dWidth = -1; dWidth < 2; dWidth++) {
				for (int dHeight = -1; dHeight < 2; dHeight++) {
					int iNew = (row + dHeight + height) % height;
					int jNew = (column + dWidth + width) % width;
					float multiplier = patternMatrix[dWidth + 1][dHeight + 1];
					if (iNew == 1 && jNew == 1) {
						newGrid[iNew][jNew] = centerCellProbability;
					}
					else {
						newGrid[iNew][jNew] += value * multiplier;
					}
				}
			}
		}
