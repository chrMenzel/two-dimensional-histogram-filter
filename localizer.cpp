...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    float belief = 1.0 / count;

    newGrid = zeros(height, width);

    for (int row = 0; row < height; row++) {
        for (int column = 0; column < width; column++) {
            newGrid[row][column] = belief;
        }
    }
  ...
  // your code here
    ...
    for (int currentRow = 0; currentRow < height; currentRow++) {
        for (int currentColumn = 0; currentColumn < width; currentColumn++) {
            newRow = (currentRow + dy) % height;
            newColumn = (currentColumn + dx) % width;

            newGrid[newRow][newColumn] = beliefs[currentRow][currentColumn];
        }
    }
	...
	float p_miss) 
{
    ...
    for (int row = 0; row < height; row++) {
        for (int column = 0; column < width; column++) {
            ...
            newGrid[row][column] = beliefs[row][column];

            if (grid[row][column] == color) {
                newGrid[row][column] *= p_hit;
            }
            else
            {
                newGrid[row][column] *= p_miss;
            }
        }
    }
